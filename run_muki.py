...
from models.uka_multiple_teacher import UKABert
from logger_callback import AKACallBack
...
class ModelArguments:
    ...
    kd_temperature: Optional[float] = field(default=1.0, metadata={"help": "KD distillation kl temperature"})
    teacher_score_temperature: Optional[float] = field(default=0.2, metadata={"help": "teacher score softmax temperature for soft-teacher"})
    eval_strategy: Optional[str] = field(default='student', metadata={"help": "eval strategy of model"})
    vkd_weight: Optional[float] = field(default=1.0, metadata={"help": "weight of vanilla KD weight"})
    hard_teacher: Optional[int] = field(default=1, metadata={"help": "1: use hard teacher selection, otherwise (0) "
                                                                     "for soft teacher"})

    patience: Optional[int] = field(default=-1, metadata={"help": "early stop patience"})
    consistency_schedule: Optional[int] = field(default=0, metadata={"help": "0: do not use consistency schedule, 1 use consistency schedule"})
    no_mc: Optional[int] = field(default=0, metadata={"help": "1: do not use monte-carlo dropout, 1 use "})
    ...
    overlap_class_number: Optional[int] = field(default=0, metadata={"help": "teacher number"})
...
def main():
    ...
    print(teacher_paths)
    assert len(teacher_paths) == model_args.teacher_number, "Teacher path split result are supposed to match teacher number"
    for i in range(model_args.teacher_number):
        t_config = AutoConfig.from_pretrained(
            teacher_paths[i],
            num_labels=teacher_num_labels[i] + model_args.overlap_class_number,
    ...
    logger.info(
        f"vkd:{model_args.vkd_weight}\n   hard teacher:{model_args.hard_teacher}\n  consistency schdule: {model_args.consistency_schedule} ")

    model = UKABert.from_pretrained(
        model_args.student_model_name_or_path,
        config=student_config,
        teachers=None,
        kd_temperature=model_args.kd_temperature,
        teacher_score_temperature=model_args.teacher_score_temperature,
        eval_strategy=model_args.eval_strategy,
        vkd_weight=model_args.vkd_weight,
        consistency_schedule=model_args.consistency_schedule,
        hard_teacher=model_args.hard_teacher,
        no_mc=model_args.no_mc,
        overlap_class_number=model_args.overlap_class_number
    )
    ...
    elif data_args.task_name in ['cnews', 'sogou']:  # both have
    ...
    if model_args.patience > 0:
        callbacks.append(EarlyStoppingCallback(early_stopping_patience=model_args.patience))
