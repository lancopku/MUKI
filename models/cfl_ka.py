...
class CFLBert(BertForSequenceClassification):
    def __init__(self, config,
        ...
        self.almal_blocks = nn.ModuleList(
            CFLFCBlock(config, teacher_configs,teacher_number=teacher_number) for _ in range(align_number))
        self.cfl_loss = CFLLoss(sigmas=None, normalized=True)
        self.kd_loss = None
        ...
        if self.training:
            ...
            with torch.no_grad():
                for t_model in self.teachers:
                    ...
                    t_hiddens.append(t_output['hidden_states'][-1:-(1 + self.align_number):-1])
            ...
            for i, block in enumerate(self.almal_blocks):
                (hs, hts), (_fts, fts) = block(student_hiddens[i], [t_hidden[i] for t_hidden in t_hiddens])
                _almal_loss, _rec_loss = self.cfl_loss(hs, hts, _fts, fts)
                almal_loss += _almal_loss
                rec_loss += _rec_loss  # amal block is supposed learned with larger lr
